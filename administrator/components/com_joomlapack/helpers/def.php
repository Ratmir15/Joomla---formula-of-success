...
ENDHTML;
		...
		else
		{
			...
			$urlbase = JURI::base().'index.php?option=com_joomlapack&view=def&folder='.urlencode($folder).'&task=toggle&item=' ;
			foreach($folders as $thisEntry)
			{
				...
				{
					$href = $urlbase.urlencode($thisEntry);
					$href2 = JURI::base().'index.php?option=com_joomlapack&view=def&folder='.urlencode($myPath);
				}
