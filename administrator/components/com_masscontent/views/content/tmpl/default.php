<?php defined('_JEXEC') or die('Restricted access');    
    ...
    ?>
	  ...
	  <script language="javascript" type="text/javascript">
		...
		function copyTitle(){
            ...
            if (document.getElementById("duplicateTitle").checked){                            
                for (i=1;i<<?php echo $this->params->get('nbMassContent')+1; ?>;i++){
        ...
        function copyMeta( meta) {
            if (document.getElementById("duplicate"+meta).checked){
              for (i=2;i<<?php echo $this->params->get('nbMassContent')+1; ?> ;i++){
       ...
       //copy 1st introtext to all the other introtext
		...
		<?php if (($editor->_name=="tinymce" || $editor->_name=="jce") && $this->params->get('displayWysiwyg')!="0") {?>
      ...
      <h1>Mass content</h1>
	  <form action="index2.php?controller=Content" method="post" name="adminForm" id="adminForm" class="adminForm">
           ...
           <script language="javascript" type="text/javascript">   
            ...
            <tr>
                <td>
                <fieldset>      
                    <legend><?php echo JText::_("CREATE UP TO")." ".$this->params->get('nbMassContent')." ". JText::_("SECTIONS IN A ROW");?></legend> 
                    <table border="0" cellpadding="3" cellspacing="0">

                    <?php $k = 0;
					for ($i=1;$i< $this->params->get('nbMassContent')+1;$i++) { 
					?>
                       <tr bgcolor="<?php echo($k==0)?"#f9f9f9":"#eeeeee";?>">					   
                            <td><?php echo JText::_("TITLE");?> (<?php echo JText::_("PAGE")." ".$i; ?>): </td>							
							<td><input class="inputbox" type="text" size="50" maxlength="255"  id="title_<?php echo $i; ?>" name="title[]" value="" ></td>   
							
						<?php if ($this->params->get('displayAlias')==1 ){ ?>							
							<td><?php echo JText::_("ALIAS");?></td>							
							<td><input class="inputbox" type="text" size="50" maxlength="255"  id="alias_<?php echo $i; ?>" name="alias[]" value="" ></td>   
						<?php } 
								else 
								$hidden.='<input type="hidden"  id="alias_<?php echo $i; ?>" name="alias[]" value=""  >'; ?>
					    </tr>
						<?php if ($this->params->get('displayIntroText')==1 ){ ?>
                         <tr bgcolor="<?php echo($k==0)?"#f9f9f9":"#eeeeee";?>">
                            <td><?php echo JText::_("INTRO_TEXT")." (".JText::_("PAGE")." ".$i; ?>): </td>
							<?php if (($i==1 && $this->params->get('displayWysiwyg')=="1") ||$this->params->get('displayWysiwyg')=="2" ) echo "<td colspan=\"4\">".$editor->display( 'introtext_'.$i, "" ,'50%', '50', '20', '50' )."</td></tr><tr>" ; 
								else {?>
	                            <td><textarea id="<?php echo 'introtext_'.$i;?>" name="<?php echo 'introtext_'.$i;?>" rows="4" cols="35"></textarea></td>
								<?php } 
							} 
							else 
								$hidden.='<input type="hidden" id="introtext_'.$i.'" name="introtext_'.$i.'" value=""  >'; ?>
							
							<?php if ($this->params->get('displayFullText')==1){ ?>
	                            <td><?php echo JText::_("FULL_TEXT")." (".JText::_("PAGE")." ".$i; ?>): </td>
								<?php if (($i==1 && $this->params->get('displayWysiwyg')=="1") ||$this->params->get('displayWysiwyg')=="2" ) echo "<td colspan=\"4\">".$editor->display( 'fulltext_'.$i, "" ,'50%', '50', '20', '50' )."</td>" ; else {?>
                            <td><textarea id="<?php echo 'fulltext_'.$i;?>" name="<?php echo 'fulltext_'.$i;?>" rows="4" cols="35"></textarea></td><?php }
							}
							else 
              ...
              <tr>	
                            ...
                            <td><textarea id="metadesc_<?php echo $i; ?>" name="metadesc[]" rows="1" cols="35"></textarea></td><?php }
							else 
                            ...
                            <td><textarea id="metakey_<?php echo $i; ?>" name="metakey[]" rows="1" cols="35"></textarea></td><?php }
							else 
